...
function renderViewer(camera=false){
 ...
 activeTracks().forEach((t,i)=>{
  ...
  if(t.url){vid.src=t.url;empty.hidden=true;vid.dataset.track=i;configureVideo(vid,i);if(state.playing)vid.play().catch(()=>{})}
...
async function record(){
 ...
 try{
  const stream=await ensureMedia();await unlockAudio();$('countIn').hidden=true;
  stopMetronome();
  const recordStartAudio=await countIn(n=>{const el=$('countIn');el.hidden=!n;if(n)el.textContent=n});
  $('countIn').hidden=true;if(state.metro)startMetronome(recordStartAudio);
  ...
  state.recorder.onstop=async()=>{
   clearTimeout(state.recordStopTimer);$('countIn').hidden=true;
...
function configureVideo(v,i){
 if(!state.tracks[i]?.url)return;
 v.loop=true;
 v.playbackRate=1;
 v.muted=!audible(i);
 v.volume=Math.max(0,Math.min(1,state.tracks[i].volume??1));
}

async function restartAtLoopBoundary(videos){
 if(!state.playing)return;
 // Nur einmal pro Loop gemeinsam neu ausrichten. Keine laufenden currentTime- oder
 // playbackRate-Korrekturen mehr: Diese waren auf Android die Hauptursache des Ruckelns.
 const master=videos[0];
 if(!master)return;
 const drifts=videos.slice(1).map(v=>Math.abs((v.currentTime||0)-(master.currentTime||0)));
 if(drifts.some(d=>d>.18)){
   videos.forEach(v=>{try{v.currentTime=master.currentTime||0}catch{}});
 }
 scheduleLoopBoundary(videos);
}

function scheduleLoopBoundary(videos){
 clearTimeout(state.loopBoundaryTimer);
 if(!state.playing)return;
 const master=videos[0];
 const remaining=Math.max(.15,loopLength()-((master?.currentTime||0)%loopLength()));
 state.loopBoundaryTimer=setTimeout(()=>restartAtLoopBoundary(videos),remaining*1000+80);
}
...
async function play(){
 ...
 videos.forEach((v,i)=>{v.pause();configureVideo(v,i);try{v.currentTime=0}catch{}});
...
function stop(reset=true){
 state.playing=false;clearInterval(state.playSyncTimer);clearTimeout(state.loopBoundaryTimer);cancelAnimationFrame(state.animationId);
 document.querySelectorAll('.cell video').forEach(v=>{if(v.src){v.pause();v.playbackRate=1;if(reset)try{v.currentTime=0}catch{}}});
